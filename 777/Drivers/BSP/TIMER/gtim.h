...
#define GTIM_TIMX_PWM_CHY_GPIO_AF           GPIO_AF3_TIM8                               /* 端口复用到TIM14 */

/* TIMX REMAP设置
 * 因为我们LED0接在PF9上, 必须通过开启TIM14的部分重映射功能, 才能将TIM14_CH1输出到PF9上
 */

#define GTIM_TIMX_PWM                       TIM8                                        /* TIM14 */
#define GTIM_TIMX_PWM_CHY                   TIM_CHANNEL_3                                /* 通道Y,  1<= Y <=4 */
#define GTIM_TIMX_PWM_CHY_CCRX              TIM8->CCR2                                  /* 通道Y的输出比较寄存器 */
#define GTIM_TIMX_PWM_CHY_CLK_ENABLE()      do{ __HAL_RCC_TIM8_CLK_ENABLE(); }while(0)  /* TIM14 时钟使能 */
...
void gtim_timx_output_chy_init(uint16_t arr, uint16_t psc);
#endif
